import { WA_ADMIN } from '@/lib/utils'

export default function Hero() {
  const waUrl = `https://wa.me/${WA_ADMIN}?text=Halo%20DevStore%2C%20saya%20mau%20tanya%20tentang%20produk`

  return (
    <section className="gradient-hero gradient-mesh-overlay relative min-h-[90vh] flex items-center overflow-hidden pt-24">
      <div className="absolute -top-24 -right-24 h-80 w-80 rounded-full bg-primary/5 blur-3xl animate-float" />
      <div className="absolute -bottom-24 -left-24 h-96 w-96 rounded-full bg-accent/5 blur-3xl animate-float-delayed" />

      <div className="relative z-10 mx-auto max-w-7xl px-4 py-20 w-full">
        <div className="mx-auto max-w-4xl text-center">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary-light px-4 py-1.5 text-sm font-medium text-primary">
            <span className="h-2 w-2 rounded-full bg-primary animate-pulse" />
            Digital Products Marketplace
          </div>

          <h1 className="font-heading text-4xl font-extrabold leading-tight text-text md:text-6xl lg:text-7xl">
            Build Faster.
            <br />
            <span className="text-primary">Ship Smarter.</span>
          </h1>

          <p className="mx-auto mt-6 max-w-2xl text-lg text-text-secondary md:text-xl">
            Template siap pakai, komponen UI premium, dan jasa pembuatan website — untuk mahasiswa, developer, dan UMKM yang ingin tampil profesional tanpa ribet.
          </p>

          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <a href="#produk" className="btn-primary text-base px-8 py-3.5">
              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
              Lihat Produk
            </a>
            <a href={waUrl} target="_blank" rel="noopener noreferrer" className="btn-outline text-base px-8 py-3.5">
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              Hubungi CS
            </a>
          </div>

          <div className="mt-16 grid grid-cols-3 gap-4 border-t border-border pt-10">
            <div className="text-center">
              <div className="font-heading text-3xl font-bold text-primary md:text-4xl">50+</div>
              <div className="mt-1 text-sm text-text-muted">Produk Digital</div>
            </div>
            <div className="text-center">
              <div className="font-heading text-3xl font-bold text-primary md:text-4xl">100+</div>
              <div className="mt-1 text-sm text-text-muted">Happy Clients</div>
            </div>
            <div className="text-center">
              <div className="font-heading text-3xl font-bold text-primary md:text-4xl">5.0</div>
              <div className="mt-1 text-sm text-text-muted">Rating &#9733;</div>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
